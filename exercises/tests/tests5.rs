...
unsafe fn modify_by_address(address: usize) {
    // SAFETY: The caller must guarantee that the address is a valid pointer
    // to a mutable `u32` and no other references exist to this memory location.
    // The pointer must be properly aligned and not null.
    unsafe {
